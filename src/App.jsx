...
export default function App() {
  ...
  // ইমেইল ও পাসওয়ার্ড দিয়ে লগইন/নিবন্ধন/রিসেট হ্যান্ডলার
  async function handleAuth(e) {
    ...
    if (loginMode === "register") {
      if (password !== confirmPassword) {
        setLoginError("❌ পাসওয়ার্ড এবং কনফার্ম পাসওয়ার্ড মেলেনি!");
        return;
      }
      try {
        ...
        setLoginError(err.message.includes("email-already-in-use") ? "❌ এই ইমেইল দিয়ে ইতিমধ্যে অ্যাকাউন্ট আছে।" : "❌ অ্যাকাউন্ট তৈরিতে সমস্যা হয়েছে।");
      }
      ...
      try {
        ...
        setLoginError("❌ ভুল ইমেইল অথবা পাসওয়ার্ড।");
      }
    } else if (loginMode === "reset") {
      try {
        // ফায়ারবেস সরাসরি ইমেইলে সুরক্ষিত রিসেট লিঙ্ক/কোড পাঠায় যা ওটিপি (OTP) এর চেয়ে বেশি নিরাপদ
        await sendPasswordResetEmail(auth, email);
        showToast("📧 পাসওয়ার্ড রিসেট লিঙ্ক ইমেইলে পাঠানো হয়েছে!");
        setLoginMode("login");
      } catch (err) {
        setLoginError("❌ এই ইমেইলটি খুঁজে পাওয়া যায়নি।");
      }
    }
  }
...
<script>window.onload=()=>window.print();<\/script>
  ...
  const S = {
    ...
    input: { width: "100%", padding: "10px 12px", border: "1.5px solid #1a1a1a", borderRadius: 8, fontSize: 14, fontFamily: "inherit", outline: "none" },
  ...
  if (authLoading) {
    return (
      <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ fontSize: 16, fontWeight: 600 }}>লোডিং হচ্ছে...</p>
      </div>
    );
  }

  // নতুন আপডেটেড লগইন স্ক্রিন (হাইড/শো, কনফার্ম পাসওয়ার্ড ও রিসেটসহ)
  if (!user) return (
    <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 8px 40px rgba(0,0,0,0.12)", padding: "30px 24px", width: "100%", maxWidth: 380, border: "2px solid #1a1a1a" }}>
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          <div style={{ fontSize: 44, marginBottom: 4 }}>🏪</div>
          <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>ডিজিটাল সেবা কেন্দ্র</h2>
          <p style={{ color: "#888", fontSize: 13, margin: "4px 0 0" }}>আপনার বিশ্বস্ত ডিজিটাল ট্র্যাকার</p>
        </div>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
          {["login", "register"].map(m => (
            <button key={m} onClick={() => { setLoginMode(m); setLoginError(""); }}
              style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "2px solid #1a1a1a", background: loginMode === m ? "#1a1a1a" : "#fff", color: loginMode === m ? "#f5f0e8" : "#1a1a1a", fontWeight: 600, cursor: "pointer", fontSize: 14, fontFamily: "inherit" }}>
              {m === "login" ? "লগইন" : "নতুন নিবন্ধন"}
            </button>
          ))}
        </div>

        <form onSubmit={handleAuth} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <input type="email" placeholder="আপনার ইমেইল (Gmail)" value={email}
            onChange={e => setEmail(e.target.value)} required style={S.input} />
          
          {loginMode !== "reset" && (
            <>
              {/* পাসওয়ার্ড ইনপুট + হাইড/শো */}
              <div style={{ position: "relative" }}>
                <input type={showPassword ? "text" : "password"} placeholder="পাসওয়ার্ড" value={password}
              ...
              {/* কনফার্ম পাসওয়ার্ড (শুধুমাত্র সাইন-আপ মোডে) */}
              {loginMode === "register" && (
                <div style={{ position: "relative" }}>
                  <input type={showConfirmPassword ? "text" : "password"} placeholder="কনফার্ম পাসওয়ার্ড" value={confirmPassword}
                    onChange={e => setConfirmPassword(e.target.value)} required style={S.input} />
                  <span onClick={() => setShowConfirmPassword(!showConfirmPassword)} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", cursor: "pointer", fontSize: 16 }}>
          ...
          {loginError && <p style={{ color: "#c0392b", fontSize: 13, textAlign: "center", margin: 0, fontWeight: 600 }}>{loginError}</p>}
          
          <button type="submit" style={{ ...S.btn(true), width: "100%", padding: "11px 0", fontSize: 15 }}>
            {loginMode === "login" ? "লগইন করুন" : loginMode === "register" ? "অ্যাকাউন্ট তৈরি করুন" : "রিসেট কোড পাঠান"}
          </button>
        </form>

        {/* ফরগট পাসওয়ার্ড বা রিসেট টগল */}
        <div style={{ textAlign: "center", marginTop: 12 }}>
          {loginMode === "login" ? (
            <span onClick={() => { setLoginMode("reset"); setLoginError(""); }} style={{ color: "#4285F4", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>পাসওয়ার্ড ভুলে গেছেন?</span>
          ) : (
            <span onClick={() => { setLoginMode("login"); setLoginError(""); }} style={{ color: "#1a1a1a", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>🔙 লগইন পেজে ফিরুন</span>
          )}
        </div>

        <hr style={{ margin: "16px 0", border: "0", borderTop: "1px dashed #bbb" }} />

        {/* গুগল লগইন বাটন */}
        <button onClick={loginWithGoogle} style={{ ...S.btn(false), width: "100%", padding: "10px 0", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          <span>🌐</span> Google অ্যাকাউন্ট দিয়ে লগইন
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.header}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ color: "#aaa", fontSize: 12 }}>👤 {user.email || user.displayName}</span>
          <button onClick={logoutUser} style={{ background: "transparent", border: "1px solid #555", color: "#ccc", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>লগআউট</button>
        </div>
      </div>
