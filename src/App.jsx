...
const DEFAULT_EXPENSE_CATS = [
  { id: 1, name: "দোকান ভাড়া", price: 0 },
  { id: 2, name: "বিদ্যুৎ বিল", price: 0 },
  { id: 3, name: "ইন্টারনেট বিল", price: 0 },
  { id: 4, name: "কাগজ/কালি/স্টেশনারি", price: 0 },
  { id: 5, name: "যাতায়াত খরচ", price: 0 },
  { id: 6, name: "অন্যান্য খরচ", price: 0 },
];
...
const EXPENSES_KEY = "shop_expenses";
const EXPENSE_CATS_KEY = "shop_expense_categories";
const NOTIFY_SETTINGS_KEY = "shop_notify_settings"; // emailjs + callmebot config
const NOTIFY_LOG_KEY = "shop_notify_log"; // log of registration notifications
...
export default function App() {
  ...
  const [expenseCats, setExpenseCats] = useState(() => {
    try { return JSON.parse(localStorage.getItem(EXPENSE_CATS_KEY)) || DEFAULT_EXPENSE_CATS; } catch { return DEFAULT_EXPENSE_CATS; }
  });
  const [expenses, setExpenses] = useState(() => {
    try { return JSON.parse(localStorage.getItem(EXPENSES_KEY)) || []; } catch { return []; }
  });
  const [notifySettings, setNotifySettings] = useState(() => {
    try { return JSON.parse(localStorage.getItem(NOTIFY_SETTINGS_KEY)) || { emailjsServiceId: "", emailjsTemplateId: "", emailjsPublicKey: "", callmebotPhone: "", callmebotApiKey: "", enabled: false }; }
    catch { return { emailjsServiceId: "", emailjsTemplateId: "", emailjsPublicKey: "", callmebotPhone: "", callmebotApiKey: "", enabled: false }; }
  });
  ...
  useEffect(() => { localStorage.setItem(EXPENSE_CATS_KEY, JSON.stringify(expenseCats)); }, [expenseCats]);
  useEffect(() => { localStorage.setItem(NOTIFY_SETTINGS_KEY, JSON.stringify(notifySettings)); }, [notifySettings]);
  ...
  function getNotifyLog() { try { return JSON.parse(localStorage.getItem(NOTIFY_LOG_KEY)) || []; } catch { return []; } }
  function pushNotifyLog(entry) {
    const log = getNotifyLog();
    localStorage.setItem(NOTIFY_LOG_KEY, JSON.stringify([entry, ...log].slice(0, 200)));
  }

  // Sends a notification to the admin (email via EmailJS + WhatsApp via CallMeBot)
  // whenever a new user registers. Requires the admin to fill in API keys under
  // অ্যাডমিন প্যানেল → নোটিফিকেশন সেটিংস. Without those keys, the event is still
  // logged in-app so the admin can see it from the Admin panel.
  async function sendAdminNotification(newUserContact) {
    const when = new Date().toLocaleString("bn-BD");
    pushNotifyLog({ contact: newUserContact, time: when, emailSent: false, whatsappSent: false });

    const cfg = (() => { try { return JSON.parse(localStorage.getItem(NOTIFY_SETTINGS_KEY)); } catch { return null; } })();
    if (!cfg || !cfg.enabled) return;

    // Email via EmailJS REST API (no SDK needed)
        ...
        await fetch("https://api.emailjs.com/api/v1.0/email/send", {
          ...
          body: JSON.stringify({
            ...
            template_params: {
              ...
              new_user: newUserContact,
              time: when,
              message: `নতুন ইউজার নিবন্ধন করেছে: ${newUserContact} (সময়: ${when})`,
            },
          }),
        });
      } catch (e) { /* network/CORS errors are silently ignored — logged below regardless */ }
    }

    // WhatsApp via CallMeBot API
    if (cfg.callmebotPhone && cfg.callmebotApiKey) {
      try {
        const text = encodeURIComponent(`🔔 ডিজিটাল সেবা কেন্দ্র: নতুন ইউজার নিবন্ধন করেছে — ${newUserContact} (${when})`);
        await fetch(`https://api.callmebot.com/whatsapp.php?phone=${cfg.callmebotPhone}&text=${text}&apikey=${cfg.callmebotApiKey}`);
      } catch (e) { /* ignored */ }
    }
  }

  function getFiltered(period, key, dataset) {
  ...
  // All unique years/months/weeks/days — works for any dataset (income or expense)
  const allYearsOf = (dataset) => [...new Set(dataset.map(t => getYearKey(t.date)))].sort().reverse();
  const allMonthsOf = (dataset, year) => [...new Set(dataset.filter(t => getYearKey(t.date) === year).map(t => getMonthKey(t.date)))].sort().reverse();
  const allWeeksOf = (dataset, month) => [...new Set(dataset.filter(t => getMonthKey(t.date) === month).map(t => getWeekKey(t.date)))].sort().reverse();
  const allDaysOf = (dataset, week) => [...new Set(dataset.filter(t => getWeekKey(t.date) === week).map(t => t.date))].sort().reverse();
  ...
  function handleAuth(e) {
    ...
    if (loginMode === "register") {
      ...
      setUser(newUser);
      setLoginError("");
      sendAdminNotification(newUser.contact);
      return;
    }
  ...
  // ===== EXPENSE ENTRY =====
  function addExpense(cat) { setEditExpense(cat); setExpQty(1); setExpPrice(String(cat.price || "")); setExpName(cat.name); }

  function confirmAddExpense() {
    const qty = parseInt(expQty) || 1;
    const price = parseFloat(expPrice) || 0;
    const tx = { id: Date.now(), date: getTodayKey(), serviceId: editExpense.id, serviceName: expName, qty, price, total: qty * price };
    setExpenses(prev => [...prev, tx]);
    setEditExpense(null);
    showToast(`✅ "${expName}" খরচ যোগ হয়েছে`);
  }

  // ===== PRINT =====
  function printReport(txs, title, subtitle, totalLabel) {
    const grouped = groupByService(txs);
    const total = sumTotal(txs);
    const label = totalLabel || "মোট আয়";
...
<tr class="total-row"><td colspan="4">${label}</td><td>৳${total}</td></tr>
</tbody></table>
...
<script>window.onload=()=>window.print();<\/script>
  ...
  const todayExp = getFiltered("day", getTodayKey(), expenses);
  const todayExpTotal = sumTotal(todayExp);
  const monthExpTotal = sumTotal(getFiltered("month", getMonthKey(getTodayKey()), expenses));
  const yearExpTotal = sumTotal(getFiltered("year", getYearKey(getTodayKey()), expenses));

  // Datasets selected by the income/expense toggle in History & Report tabs
  const historyDataset = historyType === "income" ? transactions : expenses;
  const reportDataset = reportType === "income" ? transactions : expenses;
  const allYears = allYearsOf(historyDataset);
  const allMonths = (year) => allMonthsOf(historyDataset, year);
  const allWeeks = (month) => allWeeksOf(historyDataset, month);
  const allDays = (week) => allDaysOf(historyDataset, week);
  ...
  const reportTxs = getFiltered(reportPeriod, reportKey, reportDataset);
  ...
  const navItems = [
    ...
    { key: "expense", label: "খরচ যোগ", icon: "➖" },
    ...
    { key: "manage-expense", label: "খরচ সম্পাদনা", icon: "🧾" },
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.content}>
        ...
        {tab === "dashboard" && (
          <div>
            ...
            <h2 style={S.sectionTitle}>➖ আজকের খরচ ও নিট লাভ</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 14, marginBottom: 28 }}>
              {[
                { label: "আজকের খরচ", value: `৳${bn(todayExpTotal)}`, sub: `${bn(todayExp.length)} টি এন্ট্রি`, dark: true },
                { label: "এই মাসের খরচ", value: `৳${bn(monthExpTotal)}`, sub: formatMonthKey(getMonthKey(getTodayKey())), dark: false },
                { label: "এই বছরের খরচ", value: `৳${bn(yearExpTotal)}`, sub: getYearKey(getTodayKey()) + " সাল", dark: false },
                { label: "আজকের নিট লাভ", value: `৳${bn(todayTotal - todayExpTotal)}`, sub: "আয় − খরচ", dark: false },
              ].map((s, i) => (
                <div key={i} style={{ background: s.dark ? "#1a1a1a" : "#fff", border: "2px solid #1a1a1a", borderRadius: 14, padding: "18px 20px", boxShadow: "4px 4px 0 #1a1a1a" }}>
                  <p style={{ margin: "0 0 6px", fontSize: 12, color: s.dark ? "#aaa" : "#888" }}>{s.label}</p>
                  <p style={{ margin: "0 0 4px", fontSize: 24, fontWeight: 700, color: s.dark ? "#f5f0e8" : "#1a1a1a" }}>{s.value}</p>
        ...
        {tab === "expense" && (
          <div>
            ...
            <p style={{ color: "#888", fontSize: 13, margin: "0 0 20px" }}>খরচের খাতের বাটনে ক্লিক করুন, পরিমাণ লিখে যোগ করুন।</p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 12 }}>
              {expenseCats.map(cat => (
                <button key={cat.id} className="svc-btn" onClick={() => addExpense(cat)}
                  style={{ background: "#fff", border: "2px solid #1a1a1a", borderRadius: 12, padding: "16px 14px", cursor: "pointer", textAlign: "left", fontFamily: "inherit", transition: "all .2s", boxShadow: "3px 3px 0 #1a1a1a" }}>
                  <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6, lineHeight: 1.4 }}>{cat.name}</div>
                  <div style={{ fontSize: 20, fontWeight: 700 }}>৳{bn(cat.price || 0)}</div>
                </button>
              ))}
            </div>

            <h3 style={{ margin: "24px 0 12px", fontSize: 15, fontWeight: 700 }}>আজকের খরচের এন্ট্রি ({formatDate(getTodayKey())})</h3>
            {todayExp.length === 0
              ...
              : <div style={{ background: "#fff", border: "2px solid #1a1a1a", borderRadius: 12, overflow: "hidden" }}>
                  <table style={S.table}>
                    <thead><tr>{["সময়","খাতের নাম","সংখ্যা","একক মূল্য","মোট"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                    <tbody>{todayExp.map((tx, i) => (
                      <tr key={tx.id}>
                        <td style={S.td(i)}>{new Date(tx.id).toLocaleTimeString("bn-BD", { hour: "2-digit", minute: "2-digit" })}</td>
                        <td style={S.td(i)}>{tx.serviceName}</td>
                        <td style={S.td(i)}>{bn(tx.qty)}</td>
        ...
        {tab === "report" && (
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {[["income","💰 আয়"],["expense","➖ খরচ"]].map(([k,l]) => (
                <button key={k} onClick={() => setReportType(k)} style={{ ...S.btn(reportType === k, k === "expense" ? "#e74c3c" : "#1a1a1a"), borderRadius: 20, padding: "7px 18px" }}>{l}</button>
              ))}
            </div>
            ...
            {reportGrouped.length === 0
              ...
              : <>
                  <div style={{ background: "#fff", border: "2px solid #1a1a1a", borderRadius: 12, overflow: "hidden", marginBottom: 14 }}>
                    <table style={S.table}>
                      <thead><tr>{["ক্রমিক", reportType === "income" ? "সেবার নাম" : "খাতের নাম", reportType === "income" ? "বিক্রির সংখ্যা" : "সংখ্যা", "একক মূল্য", reportType === "income" ? "মোট আয়" : "মোট খরচ"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                      <tbody>
                        ...
                        <tr style={{ borderTop: "2px solid #1a1a1a" }}>
                          <td colSpan={4} style={{ padding: "12px 14px", fontWeight: 700, fontSize: 14, background: "#f5f0e8" }}>{reportType === "income" ? "মোট আয়" : "মোট খরচ"}</td>
                          <td style={{ padding: "12px 14px", fontWeight: 700, fontSize: 16, background: "#f5f0e8" }}>৳{bn(reportTotal)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  <button onClick={() => printReport(reportTxs, `${reportPeriod === "day" ? "দৈনিক" : reportPeriod === "week" ? "সাপ্তাহিক" : reportPeriod === "month" ? "মাসিক" : "বার্ষিক"} ${reportType === "income" ? "ইনভয়েস" : "খরচের রিপোর্ট"}`, reportPeriod === "day" ? formatDate(reportKey) : reportPeriod === "week" ? formatWeekRange(reportKey) : reportPeriod === "month" ? formatMonthKey(reportKey) : reportKey + " সাল", reportType === "income" ? "মোট আয়" : "মোট খরচ")}
                    style={S.btn(true)}>
                    🖨️ {reportType === "income" ? "ইনভয়েস" : "খরচের রিপোর্ট"} প্রিন্ট / PDF করুন
                  </button>
                </>
            }

            {/* Annual drill-down: click a year to see months → click a month to see weeks → click a week to see days */}
            <div style={{ marginTop: 32 }}>
              <h3 style={{ margin: "0 0 12px", fontSize: 15, fontWeight: 700 }}>
                📆 {reportType === "income" ? "আয়ের" : "খরচের"} বার্ষিক হিস্ট্রি (ক্লিক করে বিস্তারিত দেখুন)
              </h3>
              <p style={{ color: "#888", fontSize: 12, margin: "0 0 14px" }}>
                একটি বছরে ক্লিক করলে মাস দেখাবে, একটি মাসে ক্লিক করলে সপ্তাহ দেখাবে, একটি সপ্তাহে ক্লিক করলে প্রতিদিনের হিসাব দেখাবে। (সম্পূর্ণ ইতিহাস ট্যাবেও একই ভিউ পাবেন)
              </p>
              <button onClick={() => { setHistoryType(reportType); setTab("history"); setHistoryView("years"); setHistoryYear(null); setHistoryMonth(null); setHistoryWeek(null); }}
                style={{ ...S.btn(true, reportType === "expense" ? "#e74c3c" : "#1a1a1a") }}>
                📅 {reportType === "income" ? "আয়ের" : "খরচের"} সম্পূর্ণ ইতিহাস দেখুন →
              </button>
            </div>
          </div>
        )}
        ...
        {tab === "history" && (
          <div>
            <h2 style={S.sectionTitle}>📅 সম্পূর্ণ {historyType === "income" ? "আয়ের" : "খরচের"} ইতিহাস</h2>
            <p style={{ color: "#888", fontSize: 12, margin: "0 0 14px" }}>আপনি যেদিন থেকে অ্যাকাউন্ট ব্যবহার শুরু করেছেন সেদিন থেকে আজ পর্যন্ত সকল বছর, মাস, সপ্তাহ ও দিনের হিসাব এখানে ধাপে ধাপে দেখতে পাবেন।</p>

            <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
              {[["income","💰 আয়"],["expense","➖ খরচ"]].map(([k,l]) => (
                <button key={k} onClick={() => { setHistoryType(k); setHistoryView("years"); setHistoryYear(null); setHistoryMonth(null); setHistoryWeek(null); }}
                  style={{ ...S.btn(historyType === k, k === "expense" ? "#e74c3c" : "#1a1a1a"), borderRadius: 20, padding: "7px 18px" }}>{l}</button>
              ))}
            </div>
            ...
            {historyView === "years" && (
              allYears.length === 0
                ...
                : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {allYears.map(year => {
                      const yTxs = getFiltered("year", year, historyDataset);
            ...
            {historyView === "months" && historyYear && (
              <div>
                <div style={{ background: "#f5f0e8", border: "2px solid #1a1a1a", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontWeight: 700, fontSize: 15 }}>
                  ...
                  <span style={{ float: "right", color: "#555", fontSize: 13, fontWeight: 600 }}>মোট: ৳{bn(sumTotal(getFiltered("year", historyYear, historyDataset)))}</span>
                </div>
                {allMonths(historyYear).length === 0
                  ? <p style={{ color: "#aaa" }}>কোনো ডেটা নেই</p>
                  : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {allMonths(historyYear).map(mk => {
                        const mTxs = getFiltered("month", mk, historyDataset);
            ...
            {historyView === "weeks" && historyMonth && (
              <div>
                <div style={{ background: "#f5f0e8", border: "2px solid #1a1a1a", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontWeight: 700, fontSize: 15 }}>
                  ...
                  <span style={{ float: "right", color: "#555", fontSize: 13, fontWeight: 600 }}>মোট: ৳{bn(sumTotal(getFiltered("month", historyMonth, historyDataset)))}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {allWeeks(historyMonth).map(wk => {
                    const wTxs = getFiltered("week", wk, historyDataset);
            ...
            {historyView === "days" && historyWeek && (
              <div>
                <div style={{ background: "#f5f0e8", border: "2px solid #1a1a1a", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontWeight: 700, fontSize: 15 }}>
                  ...
                  <span style={{ float: "right", color: "#555", fontSize: 13, fontWeight: 600 }}>মোট: ৳{bn(sumTotal(getFiltered("week", historyWeek, historyDataset)))}</span>
                </div>
                {allDays(historyWeek).map(day => {
                  const dTxs = getFiltered("day", day, historyDataset);
        ...
        {tab === "manage-expense" && (
          <div>
            <h2 style={{ ...S.sectionTitle }}>🧾 খরচের খাত সম্পাদনা করুন</h2>
            <p style={{ color: "#888", fontSize: 13, margin: "0 0 16px" }}>খরচের খাতের নাম বা ডিফল্ট মূল্য পরিবর্তন করুন, নতুন খাত যোগ করুন বা মুছুন।</p>
            ...
            <div style={{ background: "#fff", border: "2px dashed #1a1a1a", borderRadius: 12, padding: "16px", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 14, fontWeight: 700 }}>৳</span>
                  <input type="number" placeholder="মূল্য (ঐচ্ছিক)" value={newExpPrice} onChange={e => setNewExpPrice(e.target.value)}
                    style={{ width: 120, padding: "9px 10px", border: "1.5px solid #ddd", borderRadius: 8, fontSize: 13, fontFamily: "inherit", outline: "none" }} />
                </div>
                <button onClick={() => {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
              {expenseCats.map((cat, i) => (
                <div key={cat.id} style={{ background: "#fff", border: "2px solid #1a1a1a", borderRadius: 12, padding: "12px 16px", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <span style={{ fontSize: 13, color: "#888", minWidth: 24 }}>{i+1}.</span>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button onClick={() => { if(window.confirm("সব খরচের খাত ডিফল্টে ফিরিয়ে আনবেন?")) { setExpenseCats(DEFAULT_EXPENSE_CATS); showToast("ডিফল্টে ফিরিয়ে আনা হয়েছে"); }}} style={S.btn(false)}>🔄 ডিফল্টে ফিরুন</button>
        ...
        {tab === "admin" && isAdmin(user) && (() => {
          ...
          return (
            <div>
              ...
              <div style={{ background: "#fff", border: "2px dashed #1a1a1a", borderRadius: 12, padding: "16px 20px", marginBottom: 20 }}>
                <h3 style={{ margin: "0 0 8px", fontSize: 15, fontWeight: 700 }}>🔔 নতুন ইউজার নিবন্ধন নোটিফিকেশন সেটিংস</h3>
                <p style={{ color: "#888", fontSize: 12, margin: "0 0 14px" }}>
                  নতুন কেউ অ্যাকাউন্ট তৈরি করলে আপনি স্বয়ংক্রিয়ভাবে ইমেইল ও হোয়াটসঅ্যাপ মেসেজ পেতে নিচের তথ্য পূরণ করুন। ইমেইলের জন্য <a href="https://www.emailjs.com" target="_blank" rel="noreferrer" style={{ color: "#2563eb" }}>EmailJS</a> এবং হোয়াটসঅ্যাপের জন্য <a href="https://www.callmebot.com/blog/free-api-whatsapp-messages/" target="_blank" rel="noreferrer" style={{ color: "#25d366" }}>CallMeBot</a>-এ ফ্রি অ্যাকাউন্ট খুলে কী/আইডি নিন। কোনো একটি পূরণ না করলে শুধু ওই মাধ্যমটি কাজ করবে না — অ্যাপে নোটিফিকেশন লগ সবসময় দেখা যাবে।
                </p>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                  <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                    <input type="checkbox" checked={notifySettings.enabled}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 10, marginBottom: 14 }}>
                  {[
                    ...
                    ["callmebotPhone", "WhatsApp নম্বর (যেমন 880188...)"],
                    ["callmebotApiKey", "CallMeBot API Key"],
                  ].map(([key, label]) => (
                    <div key={key}>
                      <label style={{ display: "block", fontSize: 12, color: "#555", marginBottom: 4 }}>{label}</label>
                      <input type="text" value={notifySettings[key] || ""} onChange={e => setNotifySettings(s => ({ ...s, [key]: e.target.value }))} style={S.input} />
                    </div>
                  ))}
                </div>
                <button onClick={() => showToast("✅ নোটিফিকেশন সেটিংস সংরক্ষণ হয়েছে")} style={S.btn(true)}>💾 সেটিংস সংরক্ষণ করুন</button>
              </div>

              {/* Notification log */}
              <div style={{ background: "#fff", border: "2px solid #1a1a1a", borderRadius: 12, overflow: "hidden", marginBottom: 20 }}>
                <div style={{ background: "#1a1a1a", color: "#f5f0e8", padding: "12px 18px", fontWeight: 700, fontSize: 15 }}>
                  📨 নতুন নিবন্ধনের নোটিফিকেশন লগ
                </div>
                {getNotifyLog().length === 0
                  ? <p style={{ padding: 20, color: "#aaa" }}>এখনো কোনো নোটিফিকেশন নেই</p>
                  : <table style={S.table}>
                      <thead><tr><th style={S.th}>সময়</th><th style={S.th}>ইউজার</th></tr></thead>
                      <tbody>
                        {getNotifyLog().map((n, i) => (
                          <tr key={i}><td style={S.td(i)}>{n.time}</td><td style={{ ...S.td(i), fontWeight: 600 }}>{n.contact}</td></tr>
                        ))}
                      </tbody>
                    </table>
                }
      ...
      {editExpense && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "28px 24px", width: "100%", maxWidth: 380, border: "2px solid #1a1a1a", boxShadow: "6px 6px 0 #1a1a1a" }}>
            <h3 style={{ margin: "0 0 20px", fontSize: 17, fontWeight: 700 }}>খরচ যোগ করুন</h3>
            {[
              { label: "খাতের নাম", type: "text", val: expName, set: setExpName },
              { label: "মূল্য (৳)", type: "number", val: expPrice, set: setExpPrice },
              { label: "সংখ্যা", type: "number", val: expQty, set: setExpQty, min: 1 },
            ].map(f => (
              <div key={f.label} style={{ marginBottom: 14 }}>
                <label style={{ display: "block", fontSize: 13, color: "#555", marginBottom: 4 }}>{f.label}</label>
                <input type={f.type} value={f.val} min={f.min} onChange={e => f.set(e.target.value)} style={S.input} />
            ...
            <div style={{ background: "#f5f0e8", border: "1.5px solid #ddd", borderRadius: 8, padding: "10px 14px", marginBottom: 20, textAlign: "center" }}>
              <span style={{ fontSize: 14, color: "#555" }}>মোট: </span>
              <span style={{ fontSize: 22, fontWeight: 700 }}>৳{bn((parseFloat(expPrice)||0) * (parseInt(expQty)||1))}</span>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setEditExpense(null)} style={{ ...S.btn(false), flex: 1 }}>বাতিল</button>
              <button onClick={confirmAddExpense} style={{ ...S.btn(true, "#e74c3c"), flex: 1 }}>✅ যোগ করুন</button>
            </div>
          </div>
        </div>
      )}
